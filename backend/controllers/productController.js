  ...
  getAllProducts: async (req, res) => {
    try {
      ...
      const productsWithBase64Images = products.map((product) => {
        if (product.image) {
          //console.log('Where are log ');  // Log the image data for debugging

          //console.log('Image data:', product.image);  // Log the image data for debugging
          if (Buffer.isBuffer(product.image)) { // Check if it's a Buffer
            const base64Image = `data:image/jpeg;base64,${product.image.toString('base64')}`;
            product.image = base64Image;
          } else {
            console.error('Product image is not a Buffer:', product.image);
          }
        }
        return product;
      });
      
//console.log("After using base64",productsWithBase64Images)
  ...
  getProductById: async (req, res) => {
   ...
   // console.log(id1)
    try {
      // Use findOne to get a single product by id1
